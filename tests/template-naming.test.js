...
test('unified admin console includes all service management surfaces', () => {
    ...
    assert.match(adminHtml, /\['relay','codebuddy','qoder'\]\.includes\(tab\)\)\{setNav\(tab\);if\(!options\.skipHash\)syncHashRoute\(tab\)/);
    ...
    assert.match(adminHtml, /const consoleTitle=\{relay:'Relay',codebuddy:'CodeBuddy',qoder:'Qoder'\}\[tab\]\|\|'服务管理'/);
